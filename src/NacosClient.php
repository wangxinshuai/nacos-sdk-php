...
class NacosClient
{
    ...
    protected function request(string $method, string $uri, array $options = [])
    {
		...
		if (!empty($this->accessToken)) {
			$options['query']['accessToken'] = $this->accessToken;
		}
        ...
        $client = new Client([
            'base_uri' => "{$schema}://{$this->host}:{$this->port}",
	...
	public function login(string $username, string $password)
	{
		...
		$resp = $this->request('POST', '/nacos/v1/auth/users/login', ['query' => $query]);
        $array = json_decode((string) $resp->getBody(), true);

		if (isset($array['accessToken'])) {
